...
// Fotos secundarias
$fotos_secundarias = [];
$sqlFotos = "SELECT IdFoto, Foto, Alternativo FROM Fotos WHERE Anuncio = ? ORDER BY IdFoto";
    ...
    <div class="fotos_secundarias">
        <?php if (!empty($fotos_secundarias)): ?>
            ...
            <?php foreach ($fotos_secundarias as $f): ?>
                <div class="miniatura_wrap">
                    <img class="miniatura_foto" 
                         src="<?php echo htmlspecialchars($f['Foto']); ?>" 
                         alt="<?php echo htmlspecialchars($f['Alternativo'] ?? 'Foto adicional'); ?>">
                    <div class="miniatura_actions">
                        <a class="enlaces" href="eliminar_foto.php?idfoto=<?php echo (int)$f['IdFoto']; ?>&amp;id=<?php echo $id; ?>">Eliminar</a>
                    </div>
                </div>
            <?php endforeach; ?>
        <?php else: ?>
